...
class VikBookingReportIstatRicestat extends VikBookingReport
{
	 ...
	 public function getReportData()
	 {
		  ...
		  foreach ($bookings as $gbook) {
			   ...
			   foreach ($guests_rows as $ind => $guests) {
					...
					//Sesso
					$gender = !empty($guests['gender'])  ? strtoupper($guests['gender']) : '';
					...
					//Data di nascita
					$dbirth = !empty($guests['bdate']) ? $guests['bdate'] : '';
					array_push($insert_row, array(
						 'key' => 'dbirth',
						 'attr' => array(
	 ...
	 private function initReservations() {
		  $q = "SELECT `setting` FROM `#__vikbooking_config` WHERE `param`='insertedres';";
		  $this->dbo->setQuery($q);
		  $this->dbo->execute();
		  if ($this->dbo->getNumRows() > 0) {
			   $s = $this->dbo->loadResult();
			   $s = !empty($s) ? json_decode($s, true) : $s;
			   return $s;
		  }
		  $q = "INSERT INTO `#__vikbooking_config` (`param`,`setting`) VALUES ('insertedres', '');";
		  $this->dbo->setQuery($q);
		  $this->dbo->execute();
		  return array();
	 }
	 ...
	 private function initConfiguration() {
		  $q = "SELECT `setting` FROM `#__vikbooking_config` WHERE `param`='loginvalsricestat';";
		  $this->dbo->setQuery($q);
		  $this->dbo->execute();
		  if ($this->dbo->getNumRows() > 0) {
			   $s = $this->dbo->loadResult();
		  ...
		  $q = "INSERT INTO `#__vikbooking_config` (`param`,`setting`) VALUES ('loginvalsricestat', '');";
		  $this->dbo->setQuery($q);
		  $this->dbo->execute();
		  return array();
	 }
	 ...
	 private function updateLogin($updatevals) {
		  ...
		  $q = "UPDATE `#__vikbooking_config` SET `setting`='" . $vals . "' WHERE `param`='loginvalsricestat';";
		  $this->dbo->setQuery($q);
		  $this->dbo->execute();
	 }
	 ...
	 private function updateReservations($updatevals) {
		  $vals = json_encode($updatevals);
		  $q = "UPDATE `#__vikbooking_config` SET `setting`='" . $vals . "' WHERE `param`='insertedres';";
		  $this->dbo->setQuery($q);
		  $this->dbo->execute();
	 }
}
